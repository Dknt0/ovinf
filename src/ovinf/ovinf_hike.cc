...
HikePolicy::HikePolicy(const YAML::Node &config) : BasePolicy(config) {
  // Read config
  ...
  worker_thread_ = std::thread(&HikePolicy::WorkerThread, this);
}

bool HikePolicy::WarmUp(RobotObservation<float> const &obs_pack) {
...
bool HikePolicy::InferUnsync(RobotObservation<float> const &obs_pack) {
  VectorT obs(single_obs_size_);
  obs.setZero();
...
std::optional<HikePolicy::VectorT> HikePolicy::GetResult(const size_t timeout) {
  if (inference_done_.load()) {
...
void HikePolicy::CreateLog(YAML::Node const &config) {
...
void HikePolicy::WriteLog(RobotObservation<float> const &obs_pack) {
