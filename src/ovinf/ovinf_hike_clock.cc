#include "ovinf/ovinf_hike_clock.h"
...
HikeClockPolicy::~HikeClockPolicy() {
  exiting_.store(true);
...
HikeClockPolicy::HikeClockPolicy(const YAML::Node &config) : BasePolicy(config) {
  // Read config
  ...
  worker_thread_ = std::thread(&HikeClockPolicy::WorkerThread, this);
}

bool HikeClockPolicy::WarmUp(RobotObservation<float> const &obs_pack) {
...
bool HikeClockPolicy::InferUnsync(RobotObservation<float> const &obs_pack) {
  ...
  if (!inference_done_.load()) {
    ...
    auto test_history = [this]() {
      ...
      vis_image.convertTo(
          img8u, CV_8U,
          255.0 /
              maxVal);  // Match Python: only scale by max, do NOT subtract min
      cv::Mat img_big;
      cv::resize(img8u, img_big, cv::Size(img8u.cols * 5, img8u.rows * 5), 0, 0,
                 cv::INTER_AREA);
      cv::namedWindow("hahaha", cv::WINDOW_NORMAL);
...
std::optional<HikeClockPolicy::VectorT> HikeClockPolicy::GetResult(const size_t timeout) {
  if (inference_done_.load()) {
...
void HikeClockPolicy::PrintInfo() {
...
void HikeClockPolicy::WorkerThread() {
  if (realtime_) {
...
void HikeClockPolicy::CreateLog(YAML::Node const &config) {
...
void HikeClockPolicy::WriteLog(RobotObservation<float> const &obs_pack) {
